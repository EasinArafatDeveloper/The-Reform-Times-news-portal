...
export default function InvestigationsPage() {
  ...
  const sectionTitle = isBangla ? "অনুসন্ধানী সাংবাদিকতা ডেস্ক" : "Investigative Desk";
  const sectionDesc = isBangla ? "গভীর অনুসন্ধানী রিপোর্ট এবং তথ্য বিশ্লেষণ করুন।" : "Manage in-depth investigative dispatches and public records.";
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(isBangla ? 'আপনি কি নিশ্চিত যে এই রিপোর্টটি ডিলিট করতে চান?' : 'Are you sure you want to delete this investigative dispatch?')) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAlert({ 
          type: 'success', 
          message: isBangla ? 'রিপোর্টটি সফলভাবে ডিলিট করা হয়েছে!' : 'Investigative dispatch deleted successfully!' 
        });
      }
    } catch (err) {
    ...
    <div className="space-y-10 text-body relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest mb-1.5">
            <ShieldCheck size={16} /> Public Integrity Unit
          </div>
        ...
        <Link 
          ...
          className="flex items-center justify-center gap-2 bg-primary text-white px-6 py-3 rounded-2xl font-bold hover:bg-primary/95 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-primary/10 cursor-pointer"
        >
          <Plus size={20} />
          {isBangla ? 'নতুন ফাইল খুলুন' : 'New Investigation'}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-card p-6 border border-border rounded-[24px] flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><ShieldCheck size={24} /></div>
          <div>
            <p className="text-[10px] font-bold text-caption uppercase tracking-widest">Total Case Files</p>
        ...
        <div className="bg-card p-6 border border-border rounded-[24px] flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-caption uppercase tracking-widest">Draft Reports</p>
            <p className="text-xl font-bold text-title mt-1">
            ...
            <button
              ...
              className={cn(
                ...
                selectedStatus === status 
                  ...
                  : "bg-card text-caption border-border hover:border-primary hover:text-title"
              )}
            >
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-caption font-bold uppercase tracking-widest text-xs">
                    No investigative case files found.
                  </td>
                </tr>
