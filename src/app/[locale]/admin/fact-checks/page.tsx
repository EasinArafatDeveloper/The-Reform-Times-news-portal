...
export default function FactChecksPage() {
  ...
  const typeName = "Fact Check";
  const sectionTitle = isBangla ? "তথ্য যাচাই (ফ্যাক্ট চেক) ডেস্ক" : "Fact Check Registry";
  const sectionDesc = isBangla ? "গুজব ও ভুয়া খবরের সত্যতা যাচাই এবং রিপোর্ট প্রকাশ করুন।" : "Debunk rumors, verify assertions, and administer fact checks.";
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(isBangla ? 'আপনি কি নিশ্চিত যে এই ফ্যাক্ট চেকটি ডিলিট করতে চান?' : 'Are you sure you want to delete this fact check dispatch?')) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAlert({ 
          type: 'success', 
          message: isBangla ? 'ফ্যাক্ট চেকটি সফলভাবে ডিলিট করা হয়েছে!' : 'Fact check dispatch deleted successfully!' 
        });
      }
    } catch (err) {
    ...
    <div className="space-y-10 text-body relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-indigo-500 font-bold text-xs uppercase tracking-widest mb-1.5">
            <CheckSquare size={16} /> Truth & Integrity Registry
          </div>
        ...
        <Link 
          ...
          className="flex items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold hover:bg-indigo-700 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-indigo-600/10 cursor-pointer"
        >
          <Plus size={20} />
          {isBangla ? 'নতুন সত্যতা যাচাই কার্ড' : 'Add Fact Check'}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-card p-6 border border-border rounded-[24px] flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-indigo-500/10 text-indigo-500 flex items-center justify-center"><CheckSquare size={24} /></div>
          <div>
            <p className="text-[10px] font-bold text-caption uppercase tracking-widest">Total Debunks</p>
        ...
        <div className="bg-card p-6 border border-border rounded-[24px] flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-caption uppercase tracking-widest">Draft Registries</p>
            <p className="text-xl font-bold text-title mt-1">
            ...
            <button
              ...
              className={cn(
                ...
                selectedStatus === status 
                  ...
                  : "bg-card text-caption border-border hover:border-indigo-600 hover:text-title"
              )}
            >
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-caption font-bold uppercase tracking-widest text-xs">
                    No fact check dispatches found.
                  </td>
                </tr>
                  ...
                  <tr key={articleId} className="hover:bg-surface transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="font-bold text-title group-hover:text-indigo-500 transition-colors line-clamp-1">
